 ...
 */
public class WeatherIconFactory extends LabeledFactory {
    WeatherIconFactory() {
        super(WeatherIcon)
    }
    ...
    public Object newInstance(FactoryBuilderSupport builder, Object name, Object value, Map attributes) {
        ...
        if (iconName instanceof CharSequence) {
            WeatherIcon icon = new WeatherIcon(iconName.toString())
            icon.iconSize = iconSize.toInteger()
            return icon
        } else if (iconName instanceof WeatherIconFont) {
            WeatherIcon icon = new WeatherIcon((WeatherIconFont) iconName)
            icon.iconSize = iconSize.toInteger()
            return icon
        }

        throw new IllegalArgumentException("$name cannot parse $iconName as a weathericons icon description.")
    }
}
